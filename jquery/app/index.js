...
class Game {
    ...
    setup_game() {
        ...
        let button = $('<button class="btn btn-outline-primary"/>').appendTo(
            this.game_card
        );
...
class Board {
    ...
    check_word(word) {
        /** check word on the server and append to guesses */
        let guesses = $("#guesses");
            ...
            success: data => {
                ...
                if (data.is_valid) {
                    ...
                    if (!this.guesses.includes(word)) {
                        // Only when not already guessed.
                        // Valid word and not guessed. Adding points to score.
                        this.points += data.points;
                        $("#points").text(
                            "Total points scored: " + this.points
                        );
    ...
    is_inbound(board_position) {
        /** check if next board position is adjecent */
    ...
    on_mouse_down() {
        /** table mouse down event */
        this.mouse_down = true;
    }

    on_mouse_up() {
        /** table mouse up event */
        if (this.mouse_down) {
    ...
    update_time() {
        ...
        if (this.time - time_diff < 10) {
            $("#timer").text(
                Math.round(this.time - time_diff) + " second remaining!"
            );
        ...
        if (time_diff > this.time) {
            $("#timer").text(
                Math.round(this.time - time_diff) + " Time is up!"
            );
